  ...
  <xsl:template match="t:ab">
      ...
      <div class="textpart">
          <span class="ab">
             ...
             <xsl:if test="not($parm-leiden-style=('ddbdp','sammelbuch'))                  and descendant::t:lb[last()][@rend='left-to-right']">
                <xsl:text>&#160;&#160;→</xsl:text>
             </xsl:if>
             <xsl:if test="not($parm-leiden-style=('ddbdp','sammelbuch'))                  and descendant::t:lb[last()][@rend='right-to-left']">
                <xsl:text>&#160;&#160;←</xsl:text>
             </xsl:if>
