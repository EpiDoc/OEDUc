...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:t="http://www.tei-c.org/ns/1.0" xmlns:f="http://example.com/ns/functions" xmlns:html="http://www.w3.org/1999/html" exclude-result-prefixes="t f" version="2.0">
	<!--
			...
			<xsl:when test="$parm-bib = 'zotero'">
				<xsl:choose>
					...
					<xsl:when test=".[t:ptr]">
						...
						<xsl:variable name="biblentry" select="         if ($parm-zoteroNS)         then          concat($parm-zoteroNS, substring-after(./t:ptr/@target, '#'))         else          substring-after(./t:ptr/@target, '#')"/>
