  ...
  <!-- latinnum span added in htm-teinum.xsl -->
   ...
   <xsl:template match="t:num">
      ...
      <xsl:choose>
         ...
         <xsl:when test="$parm-edition-type='interpretive' and ancestor::t:*[@xml:lang][1][@xml:lang = 'grc']             and ((number(@value) and not(contains(@value,'/')))             or (number(@atLeast) and not(contains(@atLeast,'/')))             or (number(@atMost) and not(contains(@atMost,'/'))))">
